...
/*    c3_chart_internal_fn.getY = function (min, max, domain) {
    ...
    c3_chart_internal_fn.getY = function (min, max, domain) {
        // var scale = this.getScale(min, max);
        // true signifies time series which returns this.d3.time.scale()

        var scale = this.getScale(min, max, this.isYaxisTimeSeries());
        // var scale = this.getScale(min, max, true);

        if (domain) { scale.domain(domain); }
        // scale = scale.domain([264000, 440000]).range([320, 0]);

        // scale.range([320, 0]);

        /*var scale = this.d3.time.scale()
            .domain([264000, 440000])
            .range([320, 0]);
*/
        return scale;
    };
    ...
    c3_chart_internal_fn.getYAxis = function (scale, orient, tickFormat, ticks) {
      if (this.isYaxisTimeSeries()) {
        ...
        return c3_axis(this.d3).scale(scale).orient(orient).tickFormat(tickFormat).ticks(this.d3.time[timeValue], timeInterval);
      } else {
